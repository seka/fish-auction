...
describe('SignupPage', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(useRouter).mockReturnValue({ push: mockPush } as unknown);
  });
  ...
  it('submits form with valid data', async () => {
    vi.mocked(buyerAuth.signupBuyer).mockResolvedValue({} as unknown);
  ...
  it('handles API errors (409 Conflict)', async () => {
    vi.mocked(buyerAuth.signupBuyer).mockRejectedValue({
      response: { status: 409 },
    } as unknown);
    ...
    await waitFor(() => {
      expect(
        screen.getByText('登録に失敗しました。名前が既に使用されている可能性があります。'),
      ).toBeInTheDocument();
    });
  });
  ...
  it('handles API errors (500 Server Error)', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('この操作の実行中にエラーが発生しました。運営にお問い合わせください'),
      ).toBeInTheDocument();
    });
  });
});
