...
func TestAdminAuthMiddleware_Success(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: "admin_session", Value: "admin-session-1", Secure: true, HttpOnly: true})
...
func TestAdminAuthMiddleware_RoleMismatch(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(), http.MethodGet, "/api/admin/fishermen", nil)
	req.AddCookie(&http.Cookie{Name: "admin_session", Value: "buyer-session-1", Secure: true, HttpOnly: true})
	w := httptest.NewRecorder()
